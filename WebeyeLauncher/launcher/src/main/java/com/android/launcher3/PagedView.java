...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    public void scrollTo(int x, int y) {
         ...
         * Loop is forbidden for one page case. */
        if (mSlideLoop && getChildCount() >= 2) {
            int newX;
            int viewWidth = getViewportWidth();
            int halfViewWidth = viewWidth /2 ;
            if (x > (mMaxScrollX + halfViewWidth)) {
                newX = x - mMaxScrollX - viewWidth;
                mCurrentPage = 0;
                mSlideLoopOccuried = SLIDE_LOOP_TO_FIRST;
                notifyPageSwitchListener();
            } else if (x < -halfViewWidth) {
                newX = mMaxScrollX + x + viewWidth;
                mCurrentPage = getPageCount() - 1;
                mSlideLoopOccuried = SLIDE_LOOP_TO_LAST;
                notifyPageSwitchListener();
            } else {
                newX = x;
            }
            scrollToByLoop(newX, y);
            return;
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        /* Lenovo-sw luyy1 change for loop support 2015-08-06*/
        ...
        if (mFadeInAdjacentScreens || (mTransitionEffect != null && mUseTransitionEffect) || mScrollTransformsSet) {
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (v != null) {
                    ...
                    /* Lenovo-sw luyy1 add start for loop support 2015-08-06*/
                    if (mSlideLoop) {
                        if (mTransitionEffect != null && getChildCount() >= 2) {
                            mTransitionEffect.screenScrolled(v, i, scrollProgress);
                        } else {
                            ScrolledWithoutEffect(v);
                        }
                    } else {
 ...
 */
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (pageCount > 0) {
            ...
            /* Lenovo-sw luyy1 add start for loop support 2015-08-06*/
            drawLastPage(canvas, pageCount, screenCenter);
            /* Lenovo-sw luyy1 add end for loop support 2015-08-06*/
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                mSlideLoopOccuried = SLIDE_LOOP_NONE;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (!mFreeScroll) {
                    if (mSlideLoopOccuried != SLIDE_LOOP_NONE) {
                        snapToDestination();
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                       ...
                       if (!mSlideLoop) {
                            ...
                            int newX = width * (getChildCount() - 1) ;
                            final int sX = width * getChildCount() + mUnboundedScrollX;
                            final int delta = newX-sX;
                            mUnboundedScrollX = sX;
                            snapToPage(finalPage, delta, PAGE_SNAP_ANIMATION_DURATION, false);
                        } else if (((isSignificantMove && deltaX < 0 && !isFling) || (isFling && velocityX < 0))
                                && mCurrentPage == getChildCount() - 1) {
                            finalPage = returnToOriginalPage ? mCurrentPage : 0;
                            int newX = 0;
                            int width = getViewportWidth();
                            final int sX = mUnboundedScrollX - width * getChildCount();
                            final int delta = newX - sX;
                            mUnboundedScrollX = sX;
                            snapToPage(finalPage, delta, PAGE_SNAP_ANIMATION_DURATION, false);
                        } else {
    ...
    int getDistanceFromScreenCenter(int index) {
        int screenCenter = getViewportOffsetX() + getScrollX() + (getViewportWidth() / 2);
        View layout = (View) getPageAt(index);
        int childWidth = layout.getMeasuredWidth();
        int halfChildWidth = (childWidth / 2);
        int childCenter = getViewportOffsetX() + getChildOffset(index) + halfChildWidth;
        return Math.abs(childCenter - screenCenter);
    }
    ...
    protected static abstract class TransitionEffect {
        ...
        public static class Zoom extends TransitionEffect {
            ...
            public void screenScrolled(View v, int i, float scrollProgress) {
                ...
                /* Lenovo-sw luyy1 add start to fix LANCHROW-202 2015-07-06*/
                if (v instanceof CellLayout) {
                    ((CellLayout) v).getShortcutsAndWidgets().setAlpha(1f);
                } else {
                    v.setAlpha(1f);
                }
                /* Lenovo-sw luyy1 add end to fix LANCHROW-202 2015-07-06*/
            }
        }
        ...
        public static class Cube extends TransitionEffect {
            ...
            public void screenScrolled(View v, int i, float scrollProgress) {
                ...
                /* Lenovo-sw luyy1 change start to fix KOLEOSROW-2605 2015-09-22
                 * Both cube in and cube out should set camera distance to scroll smoothly.*/
                /*if (mIn) {
                    v.setCameraDistance(mPagedView.mDensity * PagedView.CAMERA_DISTANCE);
                }*/
                v.setCameraDistance(mPagedView.mDensity * PagedView.CAMERA_DISTANCE);
                /* Lenovo-sw luyy1 change end to fix KOLEOSROW-2605 2015-09-22*/
	...
	public boolean animateChildToPosition(View item, int screenOrder, int cellX, int cellY,
    ...
    protected boolean mSlideLoop = false;
    ...
    private static final int SLIDE_LOOP_NONE = 0;
    private static final int SLIDE_LOOP_TO_FIRST = 1;
    private static final int SLIDE_LOOP_TO_LAST = 2;
    private int mSlideLoopOccuried;

    private void scrollToByLoop(int x, int y) {
        mUnboundedScrollX = x;
        mOverScrollX = x;
        super.scrollTo(x, y);
        mTouchX = x;
    ...
    private void ScrolledWithoutEffect(View v) {
        v.setPivotX(v.getMeasuredWidth() * 0.5f);
        v.setPivotY(v.getMeasuredHeight() * 0.5f);
        v.setRotation(0);
        v.setRotationX(0);
    ...
    private void drawLastPage(Canvas canvas, int pageCount, int screenCenter) {

        if (!mSlideLoop || pageCount < 2) {
            return;
        }

        if (mTouchState == TOUCH_STATE_SCROLLING || mNextPage != INVALID_PAGE) {
            CellLayout lastchild = (CellLayout) getChildAt(getChildCount() - 1);
            int rightEdge = lastchild.getLeft() + lastchild.getMeasuredWidth() - mFirstChildLeft;
            rightEdge += (getPaddingLeft() + getPaddingRight());
            // In order to draw the transition pages, here we need:
            // 1. Translate the canvas;
            // 2. Enforce the transition effect if using effect;
            // 3. Draw child for the given page.
            if (getScrollX() < 0) {
                if (mTransitionEffect != null) {
                    canvas.translate(-rightEdge, 0);
                    canvas.save();
                    // Here we need to add transition effect for the last page.
                    float scrollProgress = getScrollProgress(screenCenter, getChildAt(0), 0);
                    /* Lenovo-sw luyy1 change start for Zoom out effect 2015-09-30*/
                    if (mTransitionEffect.getName().equals(PagedView.TransitionEffect.TRANSITION_EFFECT_ZOOM_OUT) && Math.abs(scrollProgress) < 10E-3) {
                        // Do nothing.
                    } else {
                        mTransitionEffect.screenScrolled(getChildAt(getChildCount() - 1), getChildCount() - 1, 1 + scrollProgress);
                        drawChild(canvas,getChildAt(getChildCount() - 1),getDrawingTime());
                    }
                    //drawChild(canvas,getChildAt(getChildCount() - 1),getDrawingTime());
                    /* Lenovo-sw luyy1 change end for Zoom out effect 2015-09-30*/
                    canvas.restore();
                    canvas.translate(rightEdge, 0);
                }
                else {
                    final long drawingTime = getDrawingTime();
                    canvas.translate(-rightEdge, 0);
                    getChildAt(getChildCount() - 1).setAlpha(255);
                    getChildAt(getChildCount() - 1).setVisibility(VISIBLE);
                    drawChild(canvas, getChildAt(getChildCount() - 1), drawingTime);
                    canvas.translate(rightEdge, 0);
                }
            } else if (getScrollX() > (lastchild.getLeft() - mFirstChildLeft)) {
                if (mTransitionEffect != null) {
                    canvas.translate(rightEdge, 0);
                    canvas.save();
                    // Here we need to add transition effect for the last page.
                    float scrollProgress = getScrollProgress(screenCenter, getChildAt(getChildCount() - 1), getChildCount() - 1);
                    /* Lenovo-sw luyy1 change start for Zoom out effect 2015-09-30*/
                    if (mTransitionEffect.getName().equals(PagedView.TransitionEffect.TRANSITION_EFFECT_ZOOM_OUT) && Math.abs(scrollProgress) < 10E-3) {
                        // Do nothing.
                    } else {
                        mTransitionEffect.screenScrolled(getChildAt(0), 0, scrollProgress - 1);
