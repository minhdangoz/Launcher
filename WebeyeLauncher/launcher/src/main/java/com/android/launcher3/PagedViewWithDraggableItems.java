...
public abstract class PagedViewWithDraggableItems extends PagedView
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
    	...
    	/***
         * Lenovo-SW zhaoxin5 20150928 BLADEIIIBA-6567 START
         * 
         * 这个bug是这样的
         * 1) Android模式的添加小部件页面
         * 2) 先点击一个1*1的小部件，然后迅速点击一个较大的比如4*2的小部件
         * 3) 此时可成功的将1*1的小部件添加到桌面
         * 4) 同时桌面上会出现一个固定在所有页面的刚才同时点击的那个4*2的小部件
         * 
         * 原因如下：
         * 1) 大于1*1的小部件，会触发AppsCustomizePagedView.java的onShortPress函数
         * 2) 在AppsCustomizePagedView.java的preloadWidget函数中将自己添加到DragLayer中
         * 3) mLauncher.getDragLayer().addView(hostView);
         * 
         * 解决方法就是如下，如果当前触碰数大过2个，停止点击事件的处理
         */
        int pointerCount = ev.getPointerCount();
        if (pointerCount > 1) {
        	return true;
        }
        /** Lenovo-SW zhaoxin5 20150928 BLADEIIIBA-6567 END */
