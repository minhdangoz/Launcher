...
public class DragController {
    ...
    /**
     * Drag a list of bitmap
     * @param list The list of bitmap to display as the drag image.  It will be re-scaled to the
     *          enlarged size.
     * @param dragLayerX The x position in the DragLayer of the left-top of the list.
     * @param dragLayerY The y position in the DragLayer of the left-top of the bitmap.
     * @param source An object representing where the drag originated
     * @param dragLayout The data associated with the object that is being dragged
     * @param dragAction The drag action: either {@link #DRAG_ACTION_MOVE} or
     *        {@link #DRAG_ACTION_COPY}
     * @param dragOffset
     * @param dragRegion
     * @param initialDragViewScale
     */
    public void startDrag(List<Bitmap> list, int dragLayerX, int dragLayerY,
                          DragSource source, XDockViewLayout dragLayout, int dragAction, Point dragOffset, Rect dragRegion,
                          float initialDragViewScale) {
        if (PROFILE_DRAWING_DURING_DRAG) {
            android.os.Debug.startMethodTracing("Launcher");
        }
        Object dragInfo = dragLayout.getTag();
        ...
        for (int i = 0; i < list.size(); i++) {
            Bitmap b = list.get(i);
            DragView dragView;
            View child = dragLayout.getChildItemAt(i);
            if (child instanceof BubbleTextView) {
                dragView = new DragView(mLauncher, b, registrationX, registrationY, initialDragViewScale);
            } else {
                dragView = new DragView(mLauncher, b, registrationX, registrationY, 0, 0, b.getWidth(),
                        b.getHeight(), initialDragViewScale);
            }
            Log.d(TAG, " startDrag, list :" + mDragObject.dragViewList.size());
            if (dragOffset != null) {
                dragView.setDragVisualizeOffset(new Point(dragOffset));
            }
            if (dragRegion != null) {
            ...
            mDragObject.dragViewList.add(dragView);
        }
        for (int i = mDragObject.dragViewList.size() - 1; i >= 0; i--) {
            DragView dragView = mDragObject.dragViewList.get(i);
            dragView.show(mMotionDownX, mMotionDownY);
        }
        mLauncher.getDragLayer().performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
        handleMoveEvent(mMotionDownX, mMotionDownY);
    ...
    private void handleMoveEvent(final int x, final int y) {
        ...
        // handle viewgroup
        if (mDragObject.dragViewList != null && mDragObject.dragViewList.size() != 0) {

            for (int i = 0; i < mDragObject.dragViewList.size(); i++) {
                final int j = i;
                final DragView view = mDragObject.dragViewList.get(i);
                ...
                mHandler.postDelayed(moveRunnable, 20 * j);
            }
    ...
    private DropTarget findDropTarget(int x, int y, int[] dropCoordinates) {
    	...
    	/** Lenovo-SW zhaoxin5 20150818 XTHREEROW-435 START */
    	/*
    	 * 如果当前是在一个文件夹中拖动应用
    	 * 并且在该文件夹尚未关闭的时候松手
    	 * 则让这个应用仍然回到这个文件夹中
    	 */
    	final boolean folderOpened = (mLauncher.getWorkspace().getOpenFolder() != null);
        final boolean dragFromFolder = (mDragObject.dragSource instanceof Folder);
    	/** Lenovo-SW zhaoxin5 20150818 XTHREEROW-435 END */
        ...
        for (int i=count-1; i>=0; i--) {
            ...
            if (r.contains(x, y)) {
                // Lenovo-sw:yuanyl2, Add edit mode function.
                // 编辑态下往Menu区域拖动图标，松开手指应该弹回图标原来位置
                if (target instanceof Workspace
                        && mLauncher.getWorkspace().isInOverviewMode()
                        && mLauncher.isPositionInMenuList(x, y)) {
                    return null;
                }

            	/** Lenovo-SW zhaoxin5 20150818 XTHREEROW-435 START */
                if(target instanceof Workspace && folderOpened && dragFromFolder) {
                	return null;
                }
            	/** Lenovo-SW zhaoxin5 20150818 XTHREEROW-435 END */
    ...
    private ArrayList<DragEndListener> mDragEndListeners = new ArrayList<DragEndListener>();

    public void addDragEndListener(DragEndListener l) {
        mDragEndListeners.add(l);
    }

    public void removeDragEndListener(DragEndListener l) {
        mDragEndListeners.remove(l);
    }
    ...
    public void doDragEnd() {
        ...
        for (DragEndListener listener : mDragEndListeners) {
            listener.onAllDragEnd();
        }
    }
