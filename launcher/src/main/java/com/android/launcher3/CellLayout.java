...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mMarkSize = res.getDimensionPixelSize(R.dimen.edit_mode_mark_size);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mDrawBackground && mBackgroundAlpha > 0.0f) {
            ...
            //bg.draw(canvas);
             /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [begin]*/
            final int alpha = (int) (mBackgroundAlpha * mBackgroundAlphaMultiplier * 255);
            if (mBgClip) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
                    canvas.save(Canvas.CLIP_SAVE_FLAG);
                    updateBgClipRect(mBackgroundRect);
                    canvas.clipPath(mBgClipPath);
                    bg.draw(canvas);
                    canvas.restore();
                } else {
                    bg.setAlpha((int) (mBgClipInput * 255));
                    bg.draw(canvas);
                    bg.setAlpha(alpha);
                }
            } else {
            ...
            /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [end]*/
        }
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        /** Lenovo-SW zhaoxin5 20150826 show hotseat icon label in vibeui START */
        if (mIsHotseat && child instanceof BubbleTextView && LauncherAppState.getInstance().isCurrentAndroidMode()) {
            ((BubbleTextView) child).setTextVisibility(false);
        }
        /** Lenovo-SW zhaoxin5 20150826 show hotseat icon label in vibeui END */
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [begin]*/
        mBgClipRectCenterX = mBackgroundRect.exactCenterX();
        mBgClipRectCenterY = mBackgroundRect.exactCenterY();
        final float mBgClipRectWidth = mBackgroundRect.width();
        final float mBgClipRectHeight = mBackgroundRect.height();
        mBgClipRectRadius = FloatMath.sqrt(mBgClipRectWidth * mBgClipRectWidth + mBgClipRectHeight * mBgClipRectHeight) * .5f;
        /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [end]*/
    ...
    */
    ...
    private static int mSmallerSize = Integer.MIN_VALUE;
    private static int getSmallerSize(){
        LauncherAppState app = LauncherAppState.getInstance();
           DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
        Rect paddingL = grid.getWorkspacePadding(CellLayout.LANDSCAPE);
        Rect paddingP = grid.getWorkspacePadding(CellLayout.PORTRAIT);
        ...
        int widthL = largestSize.x - paddingL.left - paddingL.right;
        int heightL = smallestSize.y - paddingL.top - paddingL.bottom;
      
        int widthP = smallestSize.x - paddingP.left - paddingP.right;
        int heightP = largestSize.y - paddingP.top - paddingP.bottom;
        
        int countX = (int) grid.numRowsBase;//mLauncher.getWorkspace().mCellCountX;  //会有为空风险
        int countY = (int) grid.numColumnsBase;//mLauncher.getWorkspace().mCellCountY;  //会有为空风险
        
        if(true){
        	StringBuilder sb = new StringBuilder();
        	sb.append("\n");
        	sb.append("====="+grid.name+"=====start=====");
        	sb.append("\n");
        	sb.append("smallestSize{x:"+smallestSize.x+",y:"+smallestSize.y+"}");
        	sb.append("\n");
        	sb.append("largestSize{x:"+largestSize.x+",y:"+largestSize.y+"}");
        	sb.append("\n");
        	sb.append("landScape{widthL:"+widthL+",heightL:"+heightL+",width:"+widthL/countY+",height:"+heightL/countX+"}");
        	sb.append("\n");
        	sb.append("portrait{widthP:"+widthP+",heightP:"+heightP+",width:"+widthP/countY+",height:"+heightP/countX+"}");
        	sb.append("\n");
        	sb.append("cell{x:"+countX+",y:"+countY+"}");
        	sb.append("\n");
        	sb.append("====="+grid.name+"======end======");
        	sb.append("\n");
        	LauncherLog.i("xixia", sb.toString());
        }
        if(mLauncher.getResources().getBoolean(R.bool.is_tablet)) {
            return Math.min(Math.min(widthL / countY, heightL / countX),
                    Math.min(widthP / countY, heightP / countX));	
        } else {
            return Math.min(widthP / countY, heightP / countX);
        }
    }
    /* Lenovo-SW 20150316 zhaoxin5 for blade8,10,13 layout adjust. END */
    ...
    public static int[] rectToCell(int width, int height, int[] result) {
        ...
        /* Lenovo-SW 20150316 zhaoxin5 for blade8,10,13 layout adjust. START */
        if(mSmallerSize == Integer.MIN_VALUE){
        	mSmallerSize = getSmallerSize();
            if(true){
            	LauncherLog.i("xixia", "smallerSize:" + mSmallerSize);
            }
        }
        int smallerSize = mSmallerSize;//Math.min(parentWidth, parentHeight);
        ...
        spanX = Math.min(grid.numColumnsBase, spanX);
        spanY = Math.min(grid.numRowsBase, spanY);
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    public int getDesiredHeight(boolean isFolderInOverViewMode)  {
    	if(!isFolderInOverViewMode) {
    		return getDesiredHeight();
    	}
    	int countY = Math.max(mCountY, 1);
        return getPaddingTop() + getPaddingBottom() + (countY * mCellHeight) +
     ...
     * @return
     */
    static boolean findVacantCellAfterCurrentOccupied(int[] vacant, int[] current, int spanX, int spanY,
                                                      int xCount, int yCount, boolean[][] occupied) {

        int currentX = current[0];
        int currentY = current[1];

        if (currentX == xCount - 1 && currentY == yCount - 1) {
            return false;
        }
        ...
        for (int j = y; j < yCount; j++) {
            ...
            for (int i = tmp; i < xCount; i++) {
                if (i + spanX > xCount || y + spanY > yCount) {
                    return false;
                }
                available = !occupied[i][j];
                if (available) {
                    x = i;
                    y = j;
                    break;
     ...
     */
    public boolean findNextUnOccupiedCell(int[] vacant, int[] current, int spanX, int spanY) {
        return findVacantCellAfterCurrentOccupied(vacant, current, spanX, spanY,
                mCountX, mCountY, mOccupied);
    }
     ...
     * @return
     */
    static boolean findVacantCellLast(int[] vacant, int spanX, int spanY,
                                      int xCount, int yCount, boolean[][] occupied) {
        boolean available = false;
    ...
    /*
     * find previous unoccupied cell
     */
    public boolean findPreviousUnOccupiedCell(int[] vacant, int spanX,
                                              int spanY) {
        return findVacantCellLast(vacant, spanX, spanY,
                mCountX, mCountY, mOccupied);
    }
    ...
    private boolean mNotShowDelAnimFlag = false;
    ...
    private View.OnClickListener mOnDeleteEmptyScreen = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            final Workspace workspace = mLauncher.getWorkspace();
            final int MAX_SCREEN_COUNT = workspace.getMaxScreenCount();

            Log.e(TAG, "mDelImg, onClick: " + v);
            if (workspace.isInOverviewMode() && !mLauncher.mIsWidgetFlying && !mLauncher.isWorkspaceLocked()) {

                final int oldHomeIdx = workspace.getDefaultPageIndex();
                final int mIndex = workspace.getCurrentPage();
                final CellLayout cellLayout = (CellLayout) workspace.getPageAt(mIndex + 1);
            	final boolean nextIsAddPage = (null != cellLayout && cellLayout.isAddLayout());
            	...
            	Runnable r = new Runnable() {
					...
					public void run() {
		                ...
		                if (nextIsAddPage) {
		                	// 下一页是添加页面
		                	mLauncher.getWorkspace().doDelClickWhenNextPageIsAddPage(CellLayout.this, cellLayout);
		                } else {
		                	mLauncher.getWorkspace().shouldDeletePage(CellLayout.this);
		                }
		                if (mIndex < oldHomeIdx) {
		                    mLauncher.getWorkspace().setDefaultPage(oldHomeIdx - 1);
		                }
					}
				};


                if (workspace.getPageCount() == MAX_SCREEN_COUNT - 1) {
                    CellLayout cl = (CellLayout) workspace.getPageAt(MAX_SCREEN_COUNT - 2);
                    if (cl != null) {
                        if (cl.mAddImg == null || cl.mAddImg.getVisibility() == View.INVISIBLE) {
                            mLauncher.getWorkspace().addEmptyAddScreen();
                        }
                    }
                }

                CellLayout cl = (CellLayout) workspace.getPageAt(workspace.getCurrentPage());
                if (cl != null) {
                    if (cl.mDelImg != null && cl.mDelImg.getVisibility() == View.VISIBLE) {
                        cl.setBgClipEnable(true);
                        cl.setBgClipSize(mBgClipInput);
                        cl.playBgClipAnim(false, r, !nextIsAddPage);
                    }
                }
                
                /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [start]*/
                /*if (workspace.getCurrentPage() == workspace.getPageCount() - 1) {
                    CellLayout cl = (CellLayout) workspace.getPageAt(workspace.getCurrentPage());
                    if (cl != null) {
                        if (cl.mAddImg != null && cl.mAddImg.getVisibility() == View.VISIBLE) {
                            cl.setBgClipEnable(true);
                            cl.setBgClipSize(mBgClipInput);
                            cl.playBgClipAnim(false);
                        }
                    }
                }*/
                /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [end]*/
    ...
    public void setDeleteBackground(boolean flag) {
        if (mAddImg != null) {
        ...
        if (flag) {
            //用于显示x的ImageView
            if (mDelImg == null) {
                mDelImg = new ImageView(getContext());
                mDelImg.setImageResource(R.drawable.ic_delete_page);
                mDelImg.setScaleType(ImageView.ScaleType.FIT_XY);
                mDelImg.setClickable(true);
                mDelImg.setContentDescription(getResources().getString(R.string.delete_empty_page));
                mDelImg.setOnClickListener(mOnDeleteEmptyScreen);

                RelativeLayout ll = new RelativeLayout(getContext());
                RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(mMarkSize, mMarkSize);
                lp.addRule(RelativeLayout.CENTER_IN_PARENT);
                ll.addView(mDelImg, lp);

                this.addView(ll);

                /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [start]*/
                setBgClipEnable(true);
                setBgClipSize(0f);
                initBgClipAnim(true);
                if (mNotShowDelAnimFlag) {
                    mDelImg.clearAnimation();
                    mClipBgAnim.end();
                    mNotShowDelAnimFlag = false;
                }
                /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [end]*/
            } else {
    ...
    private View.OnClickListener mOnAddNewScreen = new OnClickListener() {
        @Override
        public void onClick(View v) {
            Workspace workspace = mLauncher.getWorkspace();

            Log.e(TAG, "mAddImg, onClick: " + v);
            workspace.addEmptyDelScreen();
            if (workspace.isInOverviewMode() && !mLauncher.mIsWidgetFlying
                    && !mLauncher.isWorkspaceLocked()) {

                /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [start]*/
                if (workspace.getCurrentPage() == workspace.getPageCount()-2) {
                    CellLayout cl = (CellLayout) mLauncher.getWorkspace().getPageAt(mLauncher.getWorkspace().getCurrentPage());
                    if(cl != null){
                        if(cl.mDelImg != null && cl.mDelImg.getVisibility() == View.VISIBLE){
                            cl.setBgClipEnable(true);
                            cl.setBgClipSize(mBgClipInput);
                            cl.playBgClipAnim(true, null, false);
                        }
                    }
                }
                /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [end]*/
    ...
    public void setAddBackground() {
        ...
        if (mAddImg == null) {
            // Create an add image button, and add this button to celllayout.
            mAddImg = new ImageView(context);
            mAddImg.setImageResource(R.drawable.ic_add_page);
            mAddImg.setContentDescription(res.getString(R.string.add_new_empty_page));
            mAddImg.setClickable(true);
            mAddImg.setOnClickListener(mOnAddNewScreen);

            RelativeLayout rl = new RelativeLayout(context);
            RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(mMarkSize, mMarkSize);
            lp.addRule(RelativeLayout.CENTER_IN_PARENT);
            rl.addView(mAddImg, lp);
            this.addView(rl);

            /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [start]*/
            setBgClipEnable(true);
            setBgClipSize(0f);
            initBgClipAnim(false);
            /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [end]*/
    ...
    boolean isAddLayout() {
        if(mAddImg != null && mAddImg.getVisibility() == View.VISIBLE){
            return true;
        }
        else{
            return false;
    ...
    boolean isDeleteLayout() {
        if(mDelImg != null && mDelImg.getVisibility() == View.VISIBLE){
            return true;
        }
        else{
            return false;
    ...
    public boolean isHaveItemForCell(int cellX, int cellY){
        if(cellX < mCountX && cellX >= 0 && cellY < mCountY && cellY>= 0){
    ...
    public boolean conatins(FolderRingAnimator fra) {
    ...
    private boolean mBgClip = false;

    private Path mBgClipPath = null;
    private float mBgClipRectCenterX = 0;
    private float mBgClipRectCenterY = 0;
    private float mBgClipRectRadius = 0;

    private ValueAnimator mClipBgAnim = null;
    private static final int DURATION_CLIP_BG_ANIM = 450;
    private float mBgClipInput = 0f;

    private void setBgClipEnable(boolean enable) {
        mBgClip = enable;
    }

    private void setBgClipSize(float input) {
        cancelBgClipAnim();
        setBgClipEnable(true);
        if (input > 1f) {
            input = 1f;
    ...
    boolean clipAnimDelay = false;
    public void setBgClipAnimDelay(boolean delay) {
        clipAnimDelay = delay;
    }

    private void initBgClipAnim(boolean open) {
        cancelBgClipAnim();
        setBgClipEnable(true);
        final QuadInterpolator mInterpolator = new QuadInterpolator(QuadInterpolator.OUT);
        mClipBgAnim = ValueAnimator.ofFloat(mBgClipInput, open ? 1f : 0f);
        mClipBgAnim.setDuration(DURATION_CLIP_BG_ANIM);
        mClipBgAnim.setInterpolator(mInterpolator);
        mClipBgAnim.addUpdateListener(new AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                mBgClipInput = (Float) animation.getAnimatedValue();
                invalidate(mBackgroundRect);
            }
        });

        if (clipAnimDelay) {
            clipAnimDelay = false;
            mClipBgAnim.setStartDelay(DURATION_CLIP_BG_ANIM);
        }
        mClipBgAnim.start();
    }

    private void playBgClipAnim(boolean open, final Runnable r, final boolean showHideDelAnime) {
        cancelBgClipAnim();
        setBgClipEnable(true);
        final QuadInterpolator mInterpolator = new QuadInterpolator(QuadInterpolator.OUT);
        mClipBgAnim = ValueAnimator.ofFloat(mBgClipInput, open ? 1f : 0f);
        ...
        mClipBgAnim.start();
        if (open) {
            RotateAnimation rotateAnim = new RotateAnimation(-45 * (1f - mBgClipInput), 0, RotateAnimation.RELATIVE_TO_SELF,
                    0.5f, RotateAnimation.RELATIVE_TO_SELF, 0.5f);
            mDelImg.clearAnimation();
            rotateAnim.setDuration(DURATION_CLIP_BG_ANIM);
            rotateAnim.setInterpolator(mInterpolator);
            ...
            mDelImg.startAnimation(rotateAnim);
        } else {
            RotateAnimation rotateAnim = new RotateAnimation(45 * mBgClipInput + 45, 45, RotateAnimation.RELATIVE_TO_SELF,
                    0.5f, RotateAnimation.RELATIVE_TO_SELF, 0.5f);
            mDelImg.clearAnimation();
            rotateAnim.setDuration(DURATION_CLIP_BG_ANIM);
            rotateAnim.setInterpolator(mInterpolator);
    ...
    private void updateBgClipRect(Rect bgBound) {
        if (mBgClipPath == null) {
            mBgClipPath = new Path();
        }

        final float radius = mBgClipRectRadius * mBgClipInput;
        mBgClipPath.rewind();
        mBgClipPath.addCircle(mBgClipRectCenterX, mBgClipRectCenterY, radius, Path.Direction.CCW);
    }

    /*Lenovo-sw [zhaoxy12] Background animation [2014-08-20] [end]*/
    
    /** Lenovo-SW zhaoxin5 20150721 add for auto-reorder START */
    public boolean animateChildToPosition(final View child, int cellX, int cellY, int duration,
            int delay, boolean permanent, boolean adjustOccupied, ValueAnimator va, boolean mustPlayAnim) {
        ShortcutAndWidgetContainer clc = getShortcutsAndWidgets();
        boolean[][] occupied = mOccupied;
        if (!permanent) {
            occupied = mTmpOccupied;
        ...
        if (clc.indexOfChild(child) != -1) {
            ...
            if (va == null) {
            	va = LauncherAnimUtils.ofFloat(child, 0f, 1f);
            }
            va.setDuration(duration);
            mReorderAnimators.put(lp, va);
    ...
    /**
     * 移除在cellX, cellY位置的VIEW
     * @param cellX
     * @param cellY
     */
    public View removeViewAt(int cellX, int cellY) {
    	View view = getChildAt(cellX, cellY);
    	if (view == null) {
    		return null;
    	}
    	removeView(view);
    ...
    public void setNeedUpdateDrawCache(boolean flag){
        mNeedUpdateDrawCache = flag;
    }
    private boolean mNeedUpdateDrawCache = true;
    /** Lenovo-SW zhaoxin5 20150721 add for auto-reorder END */
}
