...
public class PageIndicatorMarker extends FrameLayout {
    ...
    void setMarkerDrawables(int activeResId, int inactiveResId) {
        ...
        if(PageMarkerResources.mActiveDrawable != null){
            mActiveMarker.setImageDrawable(PageMarkerResources.mActiveDrawable);
        } else {
            mActiveMarker.setImageDrawable(r.getDrawable(activeResId));
        }
        if(PageMarkerResources.mInactiveDrawable != null &&
               inactiveResId != R.drawable.ic_pageindicator_add){
            mInactiveMarker.setImageDrawable(PageMarkerResources.mInactiveDrawable);
        } else {
            mInactiveMarker.setImageDrawable(r.getDrawable(inactiveResId));
        }
        /*Lenovo-sw zhangyj19 add 2015/07/29 modify theme end*/
    }
