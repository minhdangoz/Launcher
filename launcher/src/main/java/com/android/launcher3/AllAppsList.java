...
class AllAppsList {
    ...
    public AppInfo unreadNumbersChanged(Context context, ComponentName component,
        ...
        for (LauncherActivityInfoCompat launcherActivityInfoCompat : matches) {
            // LENOVO-SW: YUANYL2
            // Fix com.lenovo.ideafriend/com.lenovo.ideafriend.alias.DialtactsActivity icon changed 
            // to callLog icon issue. BLADETL-1779
...
//          launcherActivityInfoCompat.getComponentName().getPackageName())) {
            if (component.getClassName().equals(
                    launcherActivityInfoCompat.getComponentName().getClassName())) {
	...
	public ArrayList<AppInfo> getSearchItems(String content) {
        ...
        for (AppInfo info: data) {
            String title = info.title.toString().toLowerCase();
