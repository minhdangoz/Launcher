...
class LauncherClings implements OnClickListener {
    ...
    static final String OLD_XLAUNCHER_PREFERENCES_NAME = "com.lenovo.xlauncher_preferences.xml";
    String getSharedPrefsFolder() {
    	String shared_prefs_folder = mLauncher.getFilesDir().getParentFile().getAbsolutePath() + File.separator + "shared_prefs";
    	LauncherLog.i("xixia", "shared_prefs_folder : " + shared_prefs_folder);
    	return shared_prefs_folder;
    }
    
    boolean isSharedPrefsExists(String fileName) {
    	String fileNamePath = getSharedPrefsFolder() + File.separator + fileName;
    	boolean rt = new File(fileNamePath).exists();
    	LauncherLog.i("xixia", fileNamePath + " : " + rt);
    	return rt;
    }
    
    boolean isXLauncherPreferencesExists() {
    	/*isSharedPrefsExists("trebuchet_preferences.xml");
    	isSharedPrefsExists("com.android.launcher3.prefs.xml");
    	isSharedPrefsExists("DefaultPage");*/
    	return isSharedPrefsExists(OLD_XLAUNCHER_PREFERENCES_NAME);
    }
    
    boolean canShowMigrateClings() {
    	return isXLauncherPreferencesExists() && SettingsValue.isFirstMigrateFromXLauncher(mLauncher);
    }
    
    public void showLongPressClingForMigrateFromXLauncher() {
        ViewGroup root = (ViewGroup) mLauncher.findViewById(R.id.launcher);
        View cling = mInflater.inflate(R.layout.longpress_cling, root, false);
        ...
        cling.setOnLongClickListener(new OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                mLauncher.getWorkspace().enterOverviewMode();
                LauncherLog.i("xixia", "dissmiss 1");
				SettingsValue.setIsFirstMigrateFromXLauncher(mLauncher, false);
                dismissLongPressCling();
                return true;
            }
        });
        ...
        final ViewGroup content = (ViewGroup) cling.findViewById(R.id.cling_content);
        mInflater.inflate(R.layout.longpress_cling_content_migrate_from_xlauncher, content);
        content.findViewById(R.id.cling_dismiss_longpress_info).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				LauncherLog.i("xixia", "dissmiss 2");
				SettingsValue.setIsFirstMigrateFromXLauncher(mLauncher, false);
                dismissLongPressCling();
			}
		});
