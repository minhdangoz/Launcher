...
public class LauncherProvider extends ContentProvider {
    ...
    public static final String EMPTY_DATABASE_CREATED =
    ...
    public static final Uri CONTENT_FAVORITES =
            Uri.parse("content://" + AUTHORITY + "/favorites");
    public static final Uri CONTENT_WORKSPACE_SCREENS =
            Uri.parse("content://" + AUTHORITY + "/workspaceScreens");
    /* Lenovo-SW zhaoxin5 20150529 add for 2 layer support */
    ...
    /* LENOVO:yuanyl2 : Add for Multi-Language folder supported BEGIN*/
    public int bulkInsertFolderTitles(ContentValues[] values) {
        Uri uri = FolderTitleColumns.CONTENT_URI;
        ...
        try {
            ...
            LauncherLog.i("SQLiteDiskIOException", "  insert database error ");
            return 0;
        } finally {
            db.endTransaction();
        }
    ...
    /* Lenovo-SW zhaoxin5 20150527 SISLEYROWL-4362 after restore desktop, cannot change folder title START */
    private String TAG_UPDATE_FOLDER_TITLES = "updateFolderTitles";
    public void updateFoldTitles(String olderCountry, String newCountry) {
        LauncherLog.i(TAG_UPDATE_FOLDER_TITLES, " olderCountry = " + olderCountry + 
        ...
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        String queryFolders = "SELECT * FROM favorites WHERE itemType=2";
        Cursor cFolders = db.rawQuery(queryFolders, null);
        
        if(null == cFolders || cFolders.getCount() < 1) {
        	LauncherLog.i(TAG_UPDATE_FOLDER_TITLES, " updateFoldTitles getCount < 1");
            return;
        }
        
        int folderIdIndex = cFolders.getColumnIndex("_id");
        int folderTitleIndex = cFolders.getColumnIndex("title");
        
        Cursor cTitle = null;
        
        db.beginTransaction();
        try {
        	LauncherLog.i(TAG_UPDATE_FOLDER_TITLES, " cFolders count = " + cFolders.getCount());
        	while(cFolders.moveToNext()){
        		String oldFolderTitle = cFolders.getString(folderTitleIndex);
        		int oldFolderId = cFolders.getInt(folderIdIndex);
        		LauncherLog.i(TAG_UPDATE_FOLDER_TITLES, " updating folder title = " + oldFolderTitle + ", folder id = " + oldFolderId);
                if(oldFolderTitle.indexOf("'") >= 0){
                    continue;
                }
                String queryTitle = "SELECT * FROM folder_titles WHERE folder_id in " + " ( SELECT folder_id FROM folder_titles WHERE title='"
                + oldFolderTitle + "' ) " + "and country='" + newCountry + "'";
        		cTitle = db.rawQuery(queryTitle, null);

        		if(null == cTitle || cTitle.getCount() < 1) continue;

        		LauncherLog.i(TAG_UPDATE_FOLDER_TITLES, " cTitle count = " + cTitle.getCount());
        		if(!cTitle.moveToFirst()) continue;
        		
            	int titleIndex = cTitle.getColumnIndex(FolderTitle.FolderTitleColumns.TITLE);
            	String newTitle = cTitle.getString(titleIndex);
            	
            	if(null == newTitle || newTitle.isEmpty()){
            		continue;
            	}
            	
            	String updateTitle = "UPDATE favorites SET title = '" + newTitle + "' WHERE _id=" + Integer.toString(oldFolderId);
            	LauncherLog.i(TAG_UPDATE_FOLDER_TITLES, " sql = " + updateTitle);
                db.execSQL(updateTitle);
        	}
        	
            db.setTransactionSuccessful();
        } catch( SQLiteDiskIOException e) {
            LauncherLog.v("SQLiteDiskIOException", TAG_UPDATE_FOLDER_TITLES + " error ");
        } finally {
            if (cFolders != null) {
            	cFolders.close();
            }
    ...
    /* Lenovo-SW zhaoxin5 20150527 SISLEYROWL-4362 after restore desktop, cannot change folder title END */
    /* LENOVO:yuanyl2 : Add for Multi-Language folder supported END*/
    ...
    synchronized public void loadDefaultFavoritesIfNecessary(int flags) {
    /** Lenovo-SW zhaoxin5 20150711 add for recovery from lbk support END */
        ...
        if (sp.getBoolean(EMPTY_DATABASE_CREATED, false)) {
            ...
            if (loader == null && ((flags & LauncherModel.LOADER_FLAG_RECOVERY_FROM_LBK) != 0)) {
            	// 新的加载流程,从LBK中恢复桌面数据
            	// 首先清空当前桌面的数据
                LauncherLog.i("xixia", "loadDefaultFavoritesIfNecessary from XLauncherBackupFileLoader");
            	loader = LbkLoader.get(getContext(), mOpenHelper.mAppWidgetHost, mOpenHelper);
            }
            /** Lenovo-SW zhaoxin5 20150711 add for recovery from lbk support END */
            ...
            if (loader == null) {
            	// 原有的加载流程,判断条件是flag中没有LauncherModel.LOADER_FLAG_RECOVERY_FROM_LBK
                ...
                if (partner != null && partner.hasDefaultLayout()) {
                    ...
                    int workspaceResId = partnerRes.getIdentifier(
                    		Partner.getDefaultLayoutName()/*Partner.RES_DEFAULT_LAYOUT*/,
                            "xml", partner.getPackageName());
                    /* Lenovo-SW zhaoxin5 20150326 config for blade layout START */
                    if (workspaceResId != 0) {
                    	LauncherLog.i("xixia", "loadDefaultFavoritesIfNecessary 1 : load layout from partner");
            ...
            if (loader == null) {
            	LauncherLog.i("xixia", "loadDefaultFavoritesIfNecessary 2 : load layout from launcher");
    ...
    /* Lenovo-SW zhaoxin5 add for 2 layer support START */
    public void migrateLenovoLauncherDesktop() {
    	// mOpenHelper.createEmptyDB(mOpenHelper.getWritableDatabase());
        ModeSwitchHelper.clearVIBEUIModeData(mOpenHelper.getWritableDatabase());
    	mOpenHelper.migrateLenovoLauncherDesktop(mOpenHelper.getWritableDatabase(),
        		ModeSwitchHelper.getOldLenovoLauncherUri(getContext()));
    }
    /* Lenovo-SW zhaoxin5 add for 2 layer support END */
    ...
    /* Lenovo-SW zhaoxin5 add for 2 layer support */
    public void setFlagEmptyDbCreated(){
    	mOpenHelper.setFlagEmptyDbCreated();
    }
    /* Lenovo-SW zhaoxin5 add for 2 layer support */
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        public void onCreate(SQLiteDatabase db) {
            if (LOGD) LauncherLog.d(TAG, "creating new launcher database");
            ...
            db.execSQL("CREATE TABLE favorites (" +
                    ...
                    "uri TEXT," +
                    "displayMode INTEGER," +
                    //add it from launcher 6.5 for p1 upgrade
                    "needConfig INTEGER DEFAULT 0," +                    
                    "appWidgetProvider TEXT," +
                    "modified INTEGER NOT NULL DEFAULT 0," +
                    ////add it from launcher 6.5 for p1 upgrade
                    "hasActiveIcon INTEGER," +
                    ////add it from launcher 6.5 for p1 upgrade
                    "subItemType INTEGER," +
                    ...
                    // add new field of for2layerWorkspace to mark whether the item is for 2 layer solution or not
                    ",for2layerWorkspace INTEGER DEFAULT 0" +
                    // change end, xutg1, support to switch workspaces between 1 layer and 2 layers
                    ");");
            addWorkspacesTable(db);
            addFolderTitleTable(db);
        ...
        private void addWorkspacesTable(SQLiteDatabase db) {
            db.execSQL("CREATE TABLE " + TABLE_WORKSPACE_SCREENS + " (" +
                    ...
                    // changed by xutg1, support to switch workspaces between 1 layer and 2 layers
                    // add new field of for2layerWorkspace to mark whether the item is for 2 layer solution or not
                    LauncherSettings.WorkspaceScreens.SCREEN_FOR_2LAYER_WORKSPACE + " INTEGER NOT NULL DEFAULT 0," +
                    // change end, xutg1, support to switch workspaces between 1 layer and 2 layers
        ...
        /* LENOVO:yuanyl2 : Add for Multi-Language folder supported BEGIN*/
        private void addFolderTitleTable(SQLiteDatabase db) {
            db.execSQL("DROP TABLE IF EXISTS " + TABLE_FOLDER_TITLES);
            db.execSQL("CREATE TABLE " + TABLE_FOLDER_TITLES + " (" +
                    "_id INTEGER PRIMARY KEY AUTOINCREMENT," +
                    FolderTitle.FolderTitleColumns.FOLDER_ID + " INTEGER," +
                    FolderTitle.FolderTitleColumns.COUNTRY + " TEXT NOT NULL, " +
                    FolderTitle.FolderTitleColumns.TITLE + " TEXT NOT NULL" +
                    ");");
        }
        /* LENOVO:yuanyl2 : Add for Multi-Language folder supported End*/
        ...
        private void removeEmptyDatabaseCreatedFlag() {
            String spKey = LauncherAppState.getSharedPreferencesKey();
            SharedPreferences sp = mContext.getSharedPreferences(spKey, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sp.edit();
        ...
        private boolean convertDatabase(SQLiteDatabase db, Uri uri,
            ...
            if (LOGD) LauncherLog.d(TAG, "converting database from an older format, but not onUpgrade");
            ...
            if (converted) {
                ...
                if (LOGD) LauncherLog.d(TAG, "converted and now triggering widget upgrade");
                convertWidgets(db);

                // Update max item id
                mMaxItemId = initializeMaxItemId(db);
                if (LOGD) LauncherLog.d(TAG, "mMaxItemId: " + mMaxItemId);
            }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            if (LOGD) LauncherLog.d(TAG, "onUpgrade triggered: " + oldVersion);
            ...
            if (version == 16) {
                LauncherLog.w(TAG, "Found pre-11 Trebuchet, preparing update");
            ...
            if(oldVersion == 21){
            //upgrade from older x3/k5 to new x3/k5 database
                try {	
                    db.execSQL("ALTER TABLE favorites ADD needConfig INTEGER DEFAULT 0");
                    db.execSQL("ALTER TABLE favorites ADD hasActiveIcon INTEGER DEFAULT 0");
                    db.execSQL("ALTER TABLE favorites ADD subItemType INTEGER DEFAULT 0");
                    version = 26;
                } catch (SQLException e) {
                } finally {
                }
            }
            if (oldVersion < 26 && oldVersion > 21) {
            	LauncherLog.d(TAG,"upgrade from p1 begin");
                //upgrade db from p1
                try {
                    db.execSQL("ALTER TABLE favorites ADD restored INTEGER DEFAULT 0");
                    UserManagerCompat userManager = UserManagerCompat.getInstance(mContext);
                    ...
                    db.execSQL("ALTER TABLE favorites " +
                        ...
                        // otherwise, we don't have hidden, add it
                        db.execSQL("ALTER TABLE favorites ADD hidden INTEGER DEFAULT 0");
                        db.execSQL("ALTER TABLE favorites ADD for2layerWorkspace INTEGER DEFAULT 0");
                        db.execSQL("ALTER TABLE workspaceScreens ADD for2layerWorkspace INTEGER DEFAULT 0");
                } catch (SQLException e) {
                } finally {
                }
                LauncherLog.d(TAG,"upgrade from p1 end");
                removeEmptyDatabaseCreatedFlag();
                SettingsValue.setFirstLoadSettings(mContext,false);
                version = 26;
            }
            if (version != DATABASE_VERSION) {
                LauncherLog.w(TAG, "Destroying all old data.");
        ...
        public void onDowngrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            LauncherLog.w(TAG, "Database version downgrade from: " + oldVersion + " to " + newVersion +
                    ". Wiping databse.");
            createEmptyDB(db);
        }
        ...
        private boolean updateContactsShortcuts(SQLiteDatabase db) {
            ...
            try {
                ...
                if (LOGD) LauncherLog.d(TAG, "found upgrade cursor count=" + c.getCount());
                ...
                while (c.moveToNext()) {
                    ...
                    if (intentUri != null) {
                        try {
                            ...
                            LauncherLog.e(TAG, "Problem upgrading shortcut", ex);
                        } catch (URISyntaxException e) {
                            LauncherLog.e(TAG, "Problem upgrading shortcut", e);
                        }
                    }
                }
                ...
                LauncherLog.w(TAG, "Problem while upgrading contacts", ex);
                return false;
            } finally {
                db.endTransaction();
                if (c != null) {
        ...
        private void normalizeIcons(SQLiteDatabase db) {
            LauncherLog.d(TAG, "normalizing icons");
            ...
            try {
                ...
                while (c.moveToNext()) {
                    ...
                    try {
                        ...
                        if (!logged) {
                            LauncherLog.e(TAG, "Failed normalizing icon " + id, e);
                        } else {
                            LauncherLog.e(TAG, "Also failed normalizing icon " + id);
                        }
                        logged = true;
                    }
                }
                ...
                LauncherLog.w(TAG, "Problem while allocating appWidgetIds for existing widgets", ex);
            } finally {
                db.endTransaction();
                if (update != null) {
                    update.close();
            ...
            try {
                ...
                while (c != null && c.moveToNext()) {
                    ...
                    try {
                        ...
                        if (LOGD) {
                            LauncherLog.d(TAG, "allocated appWidgetId=" + appWidgetId
                        ...
                        LauncherLog.e(TAG, "Problem allocating appWidgetId", ex);
                    }
                }
...
//                        }
        ...
        private int loadFavorites(SQLiteDatabase db, WorkspaceLoader loader) {
            ...
            if(loader instanceof LbkLoader) {
            	// LBK Loader 不需要对页面进行排序,
            	// 因为它本身就是有序的
            	LauncherLog.i(TAG_FAVORITE, "loader instanceof LbkLoader");
            } else {
                Collections.sort(screenIds);	
            	LauncherLog.i(TAG_FAVORITE, "Collections.sort(screenIds)");
            }
            ...
            for (Long id : screenIds) {
                ...
                /** Lenovo-SW zhaoxin5 20150813 add for screen backup and restore support START */
                values.put(LauncherSettings.WorkspaceScreens.SCREEN_FOR_2LAYER_WORKSPACE, LauncherAppState.getInstance().getCurrentLayoutInt());
                /** Lenovo-SW zhaoxin5 20150813 add for screen backup and restore support END */
            ...
            try {
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    if (TAG_INCLUDE.equals(name)) {
                        ...
                        if (LOGD) LauncherLog.v(TAG, String.format(("%" + (2*(depth+1)) + "s<include workspace=%08x>"),
                                "", resId));
                        ...
                        if (resId != 0 && resId != workspaceResourceId) {
                            ...
                            LauncherLog.w(TAG, String.format("Skipping <include workspace=0x%08x>", resId));
                        }

                        if (LOGD) LauncherLog.v(TAG, String.format(("%" + (2*(depth+1)) + "s</include>"), ""));
                        continue;
                    }
                    ...
                    values.clear();
                    /* Lenovo-SW zhaoxin5 20150518 add for 2 layer support */
                    values.put(LauncherSettings.Favorites.FOR_2LAYER_WORKSPACE, String.valueOf(LauncherAppState.getInstance().getCurrentLayoutInt()));
                    /* Lenovo-SW zhaoxin5 20150518 add for 2 layer support */
                    ...
                    if (LOGD) {
                        ...
                        LauncherLog.v(TAG, String.format(
                        ...
                        while ((type = parser.next()) != XmlPullParser.END_TAG ||
                            ...
                            if (!added) {
                                if (TAG_FAVORITE.equals(fallback_item_name)) {
                                    ...
                                    LauncherLog.e(TAG, "Fallback groups can contain only favorites, found "
                                            + fallback_item_name);
                                }
                            }
                        }
        ...
        private void addToFolder(SQLiteDatabase db, Resources res, XmlResourceParser parser,
            ...
            while ((type = parser.next()) != XmlPullParser.END_TAG ||
                ...
                /* Lenovo-SW zhaoxin5 20150520 add for 2Layer support START */
                childValues.put(LauncherSettings.Favorites.FOR_2LAYER_WORKSPACE, String.valueOf(LauncherAppState.getInstance().getCurrentLayoutInt()));
                /* Lenovo-SW zhaoxin5 20150520 add for 2Layer support END */

                if (LOGD) {
                    final String pkg = getAttributeValue(parser, ATTR_PACKAGE_NAME);
                    final String uri = getAttributeValue(parser, ATTR_URI);
                    LauncherLog.v(TAG, String.format(("%" + (2*(folderDepth+1)) + "s<%s \"%s\">"), "",
        ...
        private long addAppShortcutByUri(SQLiteDatabase db, ContentValues values,
            ...
            try {
                ...
                LauncherLog.e(TAG, "Unable to add meta-favorite: " + intentUri, e);
                return -1;
            }
            ...
            if (wouldLaunchResolverActivity(resolved, appList)) {
                ...
                if (systemApp == null) {
                    ...
                    LauncherLog.w(TAG, "No preference or single system activity found for "
                            + metaIntent.toString());
                    return -1;
                }
                resolved = systemApp;
        ...
        private ResolveInfo getSingleSystemActivity(List<ResolveInfo> appList) {
            ...
            for (int i = 0; i < N; ++i) {
                try {
                    ...
                    LauncherLog.w(TAG, "Unable to get info about resolve results", e);
                    return null;
                }
            }
        ...
        private long addAppShortcut(SQLiteDatabase db, ContentValues values,
            ...
            if (!TextUtils.isEmpty(packageName) && !TextUtils.isEmpty(className)) {
                ...
                try {
                    ...
                    LauncherLog.w(TAG, "Unable to add favorite: " + packageName +
                            "/" + className, e);
                }
                return -1;
                ...
                LauncherLog.e(TAG, "Skipping invalid <favorite> with no component or uri");
                return -1;
            }
        }
        ...
        private long addUriShortcut(SQLiteDatabase db, ContentValues values, Resources res,
            ...
            try {
                ...
                LauncherLog.w(TAG, "Shortcut has malformed uri: " + uri);
                return -1; // Oh well
            }

            if (iconResId == 0 || titleResId == 0) {
                LauncherLog.w(TAG, "Shortcut is missing title or icon resource ID");
                return -1;
            }
        ...
        private void migrateLauncher2Shortcuts(SQLiteDatabase db, Uri uri) {
            ...
            mMaxItemId = initializeMaxItemId(db);
            mMaxScreenId = initializeMaxScreenId(db);
            if (LOGD) LauncherLog.d(TAG, "mMaxItemId: " + mMaxItemId + " mMaxScreenId: " + mMaxScreenId);
        }
        
        /* Lenovo-SW zhaoxin5 add for 2 layer support START */
        private void migrateLenovoLauncherDesktop(SQLiteDatabase db, Uri uri) {
        	
        	// 从用户安装的LenovoLauncher数据库中迁移数据
        	// 首先需要跟新当前数据库中的item和screen的最大值
        	mMaxItemId = initializeMaxItemId(db);
            mMaxScreenId = initializeMaxScreenId(db);
            ...
            if (c != null) {
                try {
                    if (c.getCount() > 0) {
                        ...
                        final HashSet<String> seenIntents = new HashSet<String>(c.getCount());
                        final HashMap<Long, Long> folderIds = new HashMap<Long,Long>();
                        ...
                        while (c.moveToNext()) {
                            ...
                            if(screen > maxScreen && container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
                            	// 保存老的LenovoLaucher数据库中的页面信息，但是记住Hotseat的页面信息和workspace是不一样的
                            	maxScreen = screen;
                            }
                            ...
                            if(itemType == LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET){
                                ...
                                LauncherLog.i("xixia", " 0 appWidgetProviderStr : " + appWidgetProviderStr);
                            ...
                            /* Lenovo-SW zhaoxin5 add for 2 layer support START */
                            values.put(LauncherSettings.Favorites.FOR_2LAYER_WORKSPACE, Mode.VIBEUI.ordinal());
                            /* Lenovo-SW zhaoxin5 add for 2 layer support END */
                            
                            // 更新老的LenovoLauncher的数据库中的项目的id信息,
                            // 如果是folder要保存它的老的id,
                            // 如果是存放于folder中的app,则要用保存的新的folder的id进行替换
                            long newId = generateNewItemId();
                            if(itemType == LauncherSettings.Favorites.ITEM_TYPE_FOLDER){
                            	long oldFolderId = c.getLong(idIndex);
                            	if(!folderIds.containsKey(oldFolderId)){
                            		folderIds.put(new Long(oldFolderId), new Long(newId));
                            		Log.i("xixia", "oldFolderId:"+oldFolderId + ", newId:"+newId);
                            	}
                            }
                            
                            values.put(LauncherSettings.Favorites._ID, newId); /* Lenovo-SW zhaoxin5 add for 2Layer*/
                            values.put(LauncherSettings.Favorites.INTENT, intentStr);
                            values.put(LauncherSettings.Favorites.TITLE, c.getString(titleIndex));
                            values.put(LauncherSettings.Favorites.ICON_TYPE,
                                    c.getInt(iconTypeIndex));
                        ...
                        // 更新存在于folder中的app的id的值
                        if(allItems.size() > 0){
                        	for (ContentValues values: allItems) {
                        		
                        		int oldContainer = values.getAsInteger(LauncherSettings.Favorites.CONTAINER);
                        		if(oldContainer == LauncherSettings.Favorites.CONTAINER_DESKTOP ||
                        				oldContainer == LauncherSettings.Favorites.CONTAINER_HOTSEAT){
                        			// 如果是存放在桌面上或者是Hotseat上的应用,
                        			// 不需要再更新它们的container信息了
                        			continue;
                        		}
                        		if(folderIds.get(new Long(oldContainer)) == null || folderIds.get(new Long(oldContainer)) <= 0){
                        			continue;
                        		}

                        		Long newContainer = folderIds.get(new Long(oldContainer));
                        		values.remove(LauncherSettings.Favorites.CONTAINER);
                        		values.put(LauncherSettings.Favorites.CONTAINER, newContainer);
                        		LauncherLog.i("xixia", "oldContainer:"+oldContainer+", newContainer:"+newContainer);
                            }
                        }
                        ...
                        if (allItems.size() > 0) {
                            ...
                            try {
                                for (ContentValues row: allItems) {
                                    if (row == null) continue;
                                    int itemType = row.getAsInteger(LauncherSettings.Favorites.ITEM_TYPE);
                                    if(itemType == LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET){
                                    	int screen = row.getAsInteger(LauncherSettings.Favorites.SCREEN);
                                    	int cellX = row.getAsInteger(LauncherSettings.Favorites.CELLX);
                                    	int cellY = row.getAsInteger(LauncherSettings.Favorites.CELLY);
                                    	int spanX = row.getAsInteger(LauncherSettings.Favorites.SPANX);
                                    	int spanY = row.getAsInteger(LauncherSettings.Favorites.SPANY);
                                    	int appWidgetId = row.getAsInteger(LauncherSettings.Favorites.APPWIDGET_ID);
                                    	String appWidgetProvider = row.getAsString(LauncherSettings.Favorites.APPWIDGET_PROVIDER);
                                    	LauncherLog.i("xixia", "{" 
                                    			+ "screen:" + screen 
                                    			+ "cellX:" + cellX
                                    			+ "cellY:" + cellY
                        ...
                        try {
                            for (int i=0; i<=maxScreen; i++) {
                                ...
                                /* Lenovo-SW zhaoxin5 add for 2 layer support START */
                                values.put(LauncherSettings.WorkspaceScreens.SCREEN_FOR_2LAYER_WORKSPACE, Mode.VIBEUI.ordinal());
                                /* Lenovo-SW zhaoxin5 add for 2 layer support END */
                                
                                values.put(LauncherSettings.WorkspaceScreens._ID, generateNewScreenId());
                                values.put(LauncherSettings.WorkspaceScreens.SCREEN_RANK, i);
                                if (dbInsertAndCheck(this, db, TABLE_WORKSPACE_SCREENS, null, values)
                                        < 0) {
                                    return;
    ...
    public static interface WorkspaceLoader {
        /**
