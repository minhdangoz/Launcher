...
public class DragView extends View {
    ...
    public DragView(Launcher launcher, Bitmap bitmap, int registrationX, int registrationY,
        ...
        mIconDrawablePaddingPx = getIconDrawablePaddingPx();
    ...
    private int mIconDrawablePaddingPx;
    private Paint mTextPaint;
    private Bitmap mTextBitmap;
    private Bitmap mCacheBitmap; // bitmap from iconCache, no recycle

    private int getIconDrawablePaddingPx() {
        LauncherAppState app = LauncherAppState.getInstance();
        DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();

        return grid.iconDrawablePaddingPx;
    }
    ...
    public DragView(Launcher launcher, Bitmap bitmap, int registrationX, int registrationY,
        ...
        mCacheBitmap = bitmap;//Bitmap.createBitmap(bitmap, left, top, width, height);
        setDragRegion(new Rect(0, 0, mCacheBitmap.getWidth(), mCacheBitmap.getHeight()));
        ...
        measure(ms, ms);
        mPaint = new Paint(Paint.FILTER_BITMAP_FLAG);
        mTextPaint = new Paint(Paint.FILTER_BITMAP_FLAG);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCacheBitmap != null && !mCacheBitmap.isRecycled()) {
            canvas.drawBitmap(mCacheBitmap, 0.0f, 0.0f, mPaint);
        } else {
            canvas.drawBitmap(mBitmap, 0.0f, 0.0f, mPaint);
        }
        if (mTextBitmap != null) {
            canvas.drawBitmap(mTextBitmap, 0.0f, (float) mBitmap.getHeight() + mIconDrawablePaddingPx, mTextPaint);
        }
        // Lenovo-sw:yuanyl2, Add edit mode function. End.
    ...
    */
        ...
        DragLayer.LayoutParams lp = new DragLayer.LayoutParams(0, 0);
        if (mCacheBitmap != null && !mCacheBitmap.isRecycled()) {
            lp.width = mCacheBitmap.getWidth();
            lp.height = mCacheBitmap.getHeight();
        } else {
            lp.width = mBitmap.getWidth();
            lp.height = mBitmap.getHeight();
        }
