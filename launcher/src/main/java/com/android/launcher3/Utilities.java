...
public final class Utilities {
   ...
   static Bitmap createIconBitmap(Drawable icon, Context context, int count) {
    ...
    static Bitmap createIconBitmap(Bitmap icon, Context context) {
        ...
        /** Lenovo-SW zhaoxin5 20150825 XTHREEROW-1063 START */
        final Resources resources = context.getResources();
        float textsize = resources.getDimension(R.dimen.infomation_count_textsize);
        
        boolean widthClose = Math.abs(sourceWidth - (textureWidth + textsize)) < 3;
        boolean heightClose = Math.abs(sourceHeight - (textureHeight + textsize/2)) < 3;
        if (widthClose && heightClose) {
        	// 这种情况说明这个Icon是经过前面的角标处理过的较大的icon,
        	// 直接返回即可
        	return icon;
        }
        /** Lenovo-SW zhaoxin5 20150825 XTHREEROW-1063 END */ 
    ...
    public static Bitmap createIconBitmap(Drawable icon, Context context) {
        synchronized (sCanvas) { // we share the statics :-(
            ...
            /** Lenovo-SW zhaoxin5 20150817 KOLEOSROW-976 START */
            // 首先将获得的Bitmap b的外围增加一圈
            // 用于绘制右上角的角标
            final Resources resources = context.getResources();
            float textsize = resources.getDimension(R.dimen.infomation_count_textsize);
    ...
    /* Lenovo-SW zhaoxin5 20150403 config for blade layout START */
    private static String sTabletType = null;
    private static String sTabletRegion = null;
    
    /* wifi */
    public static String getTabletType(){
    	if(null != sTabletType){
    		return sTabletType;
    	}
    	sTabletType = SystemProperties.get("ro.lenovo.tablet", "error");
    	LauncherLog.i("xixia", "getTabletType " + sTabletType);
    	return sTabletType;
    }
    
    /* row */
    public static String getTabletRegion(){
    	if(null != sTabletRegion){
    		return sTabletRegion;
    	}
    	sTabletRegion = SystemProperties.get("ro.lenovo.region", "error");
    	LauncherLog.i("xixia", "getTabletRegion " + sTabletRegion);
    	return sTabletRegion;
    }
    
    public static boolean isRowProduct(){
    	return null != getTabletRegion() && getTabletRegion().contains("row");
    }
    
    public static boolean isWifiProduct(){
    	return null != getTabletType() && getTabletType().contains("wifi");
    }
    
    public static boolean isDataProduct(){
    	return null != getTabletType() && getTabletType().contains("data");
    }
    
    public static boolean isCallProduct(){
    	return null != getTabletType() && getTabletType().contains("call");
    }
    
    public static String getCountryCodeForPhone() {
        String countryCode = SystemProperties.get("ro.lenovo.easyimage.code", "error");
        LauncherLog.i("xixia", "countryCode " + countryCode);
        return countryCode;
    }
    
    public static String getCountryCode() {
        String countryCode = SystemProperties.get("ro.product.countrycode", "error");
        LauncherLog.i("xixia", "countryCode " + countryCode);
        return countryCode;
    }
    /* Lenovo-SW zhaoxin5 20150403 config for blade layout END */
}
