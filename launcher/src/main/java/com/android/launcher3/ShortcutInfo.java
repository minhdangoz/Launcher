...
public class ShortcutInfo extends ItemInfo {
    ...
    void prepareDataForSDCardApp() {
    	mIcon = null; // 为空,才会重新构造一次它的Icon
    	container = ItemInfo.NO_ID; // 只有这样,才会被保存到数据库中
    }
    ...
    boolean initFlags = false;
    void initFlags(String from) {
    	
    	initFlags = true;
    	
    	Intent finalIntent = promisedIntent != null ? promisedIntent : intent;
    	final UserHandleCompat myUser = UserHandleCompat.myUserHandle();
        LauncherAppsCompat launcherApps = LauncherAppsCompat.getInstance(LauncherAppState.getInstance().getContext());
        LauncherActivityInfoCompat activityInfo = launcherApps.resolveActivity(finalIntent, myUser);
        if (activityInfo == null) {
            return;
        }
        
        flags = AppInfo.initFlags(activityInfo);
        
        LauncherLog.i("ShortcutInfo", title + " : " + flags + ", from : " + from);
        if(false) {
        	Log.i("xixia-1", title + " : " + flags + ", from : " + from);
        }
    }
