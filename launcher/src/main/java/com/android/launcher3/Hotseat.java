...
public class Hotseat extends FrameLayout {
    ...
    protected void onFinishInflate() {
        ...
        if (grid.isLandscape && !grid.isLargeTablet()) {
            mContent.setGridSize(1, numHotseatIcons);
        } else {
            mContent.setGridSize(numHotseatIcons, 1);
        }
        // changed end, xutg1, no ALL APPS icon in 1 layer workspace hotseat
    ...
    void resetGridSize() {
        ...
        if (LauncherAppState.isDisableAllApps()) {
        	...
        	// reset the padding value in 2 layer layout hotseat
        	int edgeMarginPx = getResources().getDimensionPixelSize(R.dimen.dynamic_grid_edge_margin);
            // mContent.setPadding(grid.getWorkspacePadding().left, 0, grid.getWorkspacePadding().right, 0);
        }
        if (grid.isLandscape && !grid.isLargeTablet()) {
            mContent.setGridSize(1, numHotseatIcons);
    ...
    FolderIcon creatFolderToHotseat() {
        LauncherAppState app = LauncherAppState.getInstance();
        IconCache mIconCache = app.getIconCache();

        FolderInfo fi = new FolderInfo();

        fi.cellX = mContent.getItemChildCount();
        fi.cellY = 0;
        fi.spanX = 1;
        fi.spanY = 1;
        fi.container = LauncherSettings.Favorites.CONTAINER_HOTSEAT;
        ...
        fi.title = mLauncher.getText(R.string.folder_name);

        LauncherModel.addItemToDatabase(mLauncher, fi, fi.container, fi.screenId, fi.cellX,
                fi.cellY, false);
        FolderIcon folder = FolderIcon.fromXml(R.layout.folder_icon, mLauncher,
                getLayout(), fi, mIconCache);
        mLauncher.getWorkspace().addInScreen(folder, fi.container, fi.screenId, fi.cellX, fi.cellY,
                fi.spanX, fi.spanY);
        return folder;
    }
