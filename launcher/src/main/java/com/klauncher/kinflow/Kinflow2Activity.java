...
public class Kinflow2Activity extends Activity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kinflow2);
        initView();
...
//        mGlobalCategoryRecyclerView.setAdapter(new GlobalCategoryAdapter(this, GlobalCategory.getDefaultGlobalCategoryList()));
        ...
        mNavigationRecyclerView.setAdapter(new NavigationAdapter2(Kinflow2Activity.this, navigationList));
...
//                            new JRTTAsynchronousTask(Kinflow2Activity.this, mRefreshHandler,MessageFactory.MESSAGE_WHAT_OBTAIN_TOUTIAO_API_ARTICLE).getJinRiToutiaoArticle(CardIdMap.getTouTiaoCategory(mCardInfo.getCardSecondTypeId()));
                            new JRTTAsynchronousTask(Kinflow2Activity.this, mRefreshHandler,MessageFactory.MESSAGE_WHAT_OBTAIN_TOUTIAO_API_ARTICLE).getJinRiToutiaoArticle(CardIdMap.getTouTiaoCategory(CardIdMap.CARD_TYPE_NEWS_TT_REDIAN));
                        } else {//获取token失败:通知MainControl获取失败
//                            Log.e(TAG, "handleMessage: 获取头条token失败");
//                            sendMessage2MainControl(msg);
                            Toast.makeText(Kinflow2Activity.this, "获取头条token失败", Toast.LENGTH_SHORT).show();
                        }
                    } catch (Exception e) {//发生异常:通知MainControl获取失败
//                        Log.e(TAG, "handleMessage: 处理今日头条token时,发生未知错误:" + e.getMessage());
//                        sendMessage2MainControl(msg);
                        Toast.makeText(Kinflow2Activity.this, "获取头条tokens时,发生未知异常", Toast.LENGTH_SHORT).show();
                    }
                    break;
...
//                            Log.e(TAG, "handleMessage: 获取头条失败");
                            Toast.makeText(Kinflow2Activity.this, "获取头条article失败", Toast.LENGTH_SHORT).show();
                        }
                    } catch (Exception e) {
//                        Log.e(TAG, "handleMessage: 处理今日头条article时,发生未知错误:"+e.getMessage());
                        Toast.makeText(Kinflow2Activity.this, "获取头条article时,发生未知异常", Toast.LENGTH_SHORT).show();
    ...
    public void requestJinRiTouTiao() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                new JRTTAsynchronousTask(Kinflow2Activity.this, mRefreshHandler, MessageFactory.MESSAGE_WHAT_OBTAIN_TOUTIAO_API_TOKEN).accessToken();
            }
        }).start();
    }
