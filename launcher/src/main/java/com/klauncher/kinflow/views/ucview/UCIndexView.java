package com.klauncher.kinflow.views.ucview;
...
public class UCIndexView extends FrameLayout implements TouchMoveView.TouchMoveListener {
     ...
     */
    private PageHeadView mPageHeadView;//UC头条那个title
    /**
     * 页面内容导航视图(对应UC首页的网址导航视图)
     * 包含总体分类:网址,小说,视频,奇趣,生活
     * 快捷导航:新浪,百度,凤凰,,,,,,等12个快捷导航
     */
    private PageNavigationView mPageNavigationView;
    /**
     * 内容视图的头部视图(对应UC首页新闻视图完全展示后的新闻类型导航头部部分，初始时隐藏)
     * 新闻类型的导航部分,例如:推荐,视频,热点,娱乐, 体育....
     */
    ...
    public void onTouchMoveEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                int offset = 0;
                if(mDelY > 0) {//hide，下拉
                    Log.e("MyInfo", "onTouchMoveEvent: 下拉,当前是否允许下拉=" + isPullRestoreEnable());
    ...
    public void showPageNavigationView() {
        onBackRestore();
    }

    public void hidePageNavigationView() {
        slip(-1, true);
    }
