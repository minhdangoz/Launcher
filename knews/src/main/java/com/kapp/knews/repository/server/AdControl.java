...
public class AdControl implements Parcelable {
    ...
    public boolean isNull() {
        if (TextUtils.isEmpty(adExtra)) return true;
        return false;
    }
    ...
    public static final Creator<AdControl> CREATOR = new Creator<AdControl>() {
        @Override
    ...
    public AdControl(JSONObject jsonObject) {
        ...
        try {
            ...
            */
            ...
            parseExtraJson(this.adExtra);

            if (TextUtils.isEmpty(this.adExtra)) return;
            this.adOrder = jsonObject.optInt(AD_ORDER, 0);
            this.adSID = jsonObject.optString(AD_SID);

        } catch (Exception e) {
            LogUtils.e("广告控制器数据解析时出现错误,详情: " + e.getMessage());
        }
    }

    void parseExtraJson(String adExtraJson) {
        if (TextUtils.isEmpty(adExtraJson)) {
            return;
        }
        try {
            JSONObject adExtraJsonObject = new JSONObject(adExtraJson);
            this.imageUrl = adExtraJsonObject.optString(KEY_IMAGE_URL, defaultImageUrl);
            this.clickUrl = adExtraJsonObject.optString(KEY_IMAGE_CLICK_URL, defaultClickImageUrl);
